...
- **One attempt only** allows the assessment to be run only once. The student will be warned that they will not be able to resubmit. You would usually want to provide a [run button](IAN) or other means for the user to test the code before running the actual assessment.
